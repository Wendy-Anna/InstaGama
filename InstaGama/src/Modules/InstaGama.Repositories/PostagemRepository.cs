...
namespace InstaGama.Repositories
{
    public class PostagemRepository
    {
        ...
        public async Task<List<Postagem>> GetPostageByUserIdAsync(int userId)
        {
            using (var con = new SqlConnection(_configuration["ConnectionString"]))
            {
                ...
                using (var cmd = new SqlCommand(sqlCmd, con))
                {
                    ...
                    var postagesForUser = new List<Postagem>();

                    while (reader.Read())
                    {
                        var postage = new Postagem(int.Parse(reader["Id"].ToString()),
